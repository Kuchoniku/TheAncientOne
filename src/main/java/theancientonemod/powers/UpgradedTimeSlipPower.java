...
public class UpgradedTimeSlipPower extends BasePower{
    public static final String POWER_ID = makeID(UpgradedTimeSlipPower.class.getSimpleName());
    private static final PowerType TYPE = PowerType.BUFF;
    private static final boolean TURN_BASED = false;
    public UpgradedTimeSlipPower(AbstractCreature owner, int amount) {
    ...
    public void turnShifted(int amount, boolean timeSlip) {
        if (!timeSlip) {
            ...
            this.addToBot(new SFXAction("ATTACK_WHIRLWIND"));
            this.addToBot(new VFXAction(new WhirlwindEffect(), 0.0F));
            addToBot(new TurnShiftAction(this.amount, true));
        }
    }
